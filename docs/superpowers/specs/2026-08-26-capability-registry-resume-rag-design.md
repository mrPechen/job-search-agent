...
> - search_job — искать и откликаться на вакансии
> - stats — показать статистику откликов
> - upload_resume — принять и разобрать резюме (PDF/DOCX)
> - chat — свободная переписка, ответы на вопросы
> Верни ключ одной из возможностей. Если это обычный разговор или вопрос —
> верни "chat". Если пользователь просит выполнить действие, которого нет в
> списке, — верни "none".
...
- `chat` → свободный чат через `text_model` (поведение прежнего `chat_node`).
